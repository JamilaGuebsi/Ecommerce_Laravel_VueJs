...
<button  @click="removeFromCart(c)">                         Remove                       </button>
</div>
</div> 
<div class="cart-product-price"> {{ c.product.prix }} TND</div>                   
<div class="cart-product-quantity">
<button @click="minus(c)" v-if="c.qty > 1">                       -      </button> 
 <div class="count"><strong> {{ c.qty }} 
</strong></div>
<button @click="plus(c)">+</button> 
</div>
<div class="cart-product-total-price">  
     {{ c.product.prix }} * {{ c.qty }} TND                   </div>
</div> 
  </div> 
 </div>  
...
<div class="cart-checkout"> 
 <div class="subtotal"> 
<span>Subtotal</span>
<span class="amount">{{$store.state.Articlestore.cartTotal}} TND</span>               </div> 
...
</router-link>                                                                       </div>             </div>           </div>         </div>         </div>           </div>     
</template> 
<script setup>
...
const Produits = ref([]); 
 onMounted(() => {                    getProduits();            } ); 
  ...
  const   clearCart=()=> {          store.commit("Articlestore/clearCart");        } 
const  removeFromCart=(item)=> {          store.commit("Articlestore/removeFromCart", item);        }  
const  plus=(item)=> {           Produits.value.map((pro)=>{          if(pro.id==item.product.id){  
     if(item.qty < pro.qtestock )            { 
    store.commit("Articlestore/plusCart", item);              }  
    else              
        alert("Quantité stock indisponible")          }                 }); 
                                    }   
const minus=(item)=> {                                         if(item.qty < 1 )            {    
       alert("Quantité stock non valable")            }      
        else            
     store.commit("Articlestore/minusCart", item); 
              }            
              const  getProduits=()=>{                              axios.get('/api/articles/').then(res => {                    Produits.value = res.data;                })
     ...
     .product h3 {  
                                                            ...
                                                            font-weight: 400; 
                                                              letter-spacing: 1.15px;
                                                                 background-color: #4b70e2;
                                                                    color: #f9f9f9;   border: none;   outline: none;   cursor: pointer; } 
                                                                     /* Cart */ 
                                                                     .cart-container {  
                                                                         padding: 2rem 4rem; } 
                                                                         .cart-container h2 {
        ...
        font-weight: 400;   text-transform: uppercase; }
         .cart-item, .cart-container .titles { 
                                                                                 ...
                                                                                 .cart-item .cart-product-quantity {  
                                                                                     display: flex; 
                                                                                       align-items: flex-start;   justify-content: center; 
                                                                                         width: 130px; 
            max-width: 100%;   border: 0.5px solid rgb(177, 177, 177); 
              border-radius: 5px; } 
            ...
            .cart-checkout .amount {   font-weight: 700; } .cart-checkout p {   font-size: 14px;   font-weight: 200;   margin: 0.5rem 0; } .cart-checkout button {   width: 100%;   height: 40px;   border-radius: 5px;   font-weight: 400;   letter-spacing: 1.15px;   background-color: #4b70e2;   color: #f9f9f9;   border: none;   outline: none;   cursor: pointer; } .continue-shopping, .start-shopping {   margin-top: 1rem; } .continue-shopping a, .start-shopping a {   color: gray;   text-decoration: none;   display: flex;   align-items: center; } .continue-shopping a span, .start-shopping a span {   margin-left: 0.5rem; } .cart-empty {   font-size: 20px;   margin-top: 2rem;   color: rgb(84, 84, 84);   display: flex;   flex-direction: column;   align-items: center; } .drag{   margin-top: 70px; 
            }    
            </style> 
